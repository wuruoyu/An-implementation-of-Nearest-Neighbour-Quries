 ...
 */
void pruneBranchList_upward(struct Node* node, struct Point given_point, int given_k) {
	for (int i = 0; i < node->count; i ++) {
		if (node->branch[i].rect.mindist > Nearest[given_k - 1].rect.dist) {
			for (int j = i; j < node->count; j ++)
				node->branch[i].pruned_flag = true;
			break;
		}
...
void nearestNeighborSearch(struct Node* node, struct Point given_point, int given_k) {
	// TODO: to assign node->type
	...
	if (node->type == 'L') {
		...
		for (int i = 0; i < node->count; i ++) {
			...
			for (int j = 0; j < given_k; j ++) {
				if (node->branch[i].rect.dist < Nearest[j].rect.dist) {
					for (int t = given_k - 2; t >= j; t --) {
						Nearest[t + 1] = Nearest[t];
					}
					Nearest[j] = node->branch[i];
					break;
				}
			}
		}
		...
		for (int i = 0; i < node->count; i ++) {
			if (!node->branch[i].pruned_flag) {
				...
				nearestNeighborSearch(&new_node, given_point, given_k);

				// perform upward pruning
				pruneBranchList_upward(node, given_point, given_k);
			}
		}
	}
...
int main() {
	...
	/* prompt to input the k */
	printf("%s\n", "Please input the size of k");
	if (scanf("%d", &given_k))
		printf("\n");
	else {
		printf("%s\n", "Input Error");
	...
	Nearest = (struct Node*)malloc(sizeof(struct Node) * given_k);
	for (int i = 0; i < given_k; i ++) {
		Nearest[i].rect.dist = LONG_MAX;
	}
	...
	nearestNeighborSearch(&root_node, given_point, given_k);
